...
export default function StudentDetailPage() {
  ...
  const fetchStudentData = useCallback(async () => {
    try {
      setLoading(true);
      const [studentData, subscriptionsData, attendanceData] = await Promise.all([
                ...
                <div className="ml-4">
                  ...
                  <div className="mt-1 flex flex-col sm:flex-row sm:flex-wrap sm:space-x-6">
                    ...
                    <div className="mt-2 flex items-center text-sm text-gray-500">
                      ...
                      가입일: {new Date(student.profile.join_date).toLocaleDateString()}
                    </div>
                    <div className="mt-2 flex items-center text-sm text-gray-500">
                      <ClockIcon className="mr-1.5 h-5 w-5 flex-shrink-0 text-gray-400" />
                      최근 방문: {student.profile.last_visit ? new Date(student.profile.last_visit).toLocaleString() : '없음'}
                    </div>
                  </div>
                </div>
              </div>
            ...
            <div className="px-4 py-5 sm:p-6">
              <dl className="grid grid-cols-1 gap-5 sm:grid-cols-2">
                ...
                <div>
                  ...
                  <dd className="mt-1 text-sm text-gray-900">{student.profile.gender === 'M' ? '남성' : '여성'}</dd>
                </div>
                <div>
                  <dt className="text-sm font-medium text-gray-500">생년월일</dt>
                  <dd className="mt-1 text-sm text-gray-900">{student.profile.birth_date || '-'}</dd>
                </div>
                <div>
                  <dt className="text-sm font-medium text-gray-500">비상연락처</dt>
                  <dd className="mt-1 text-sm text-gray-900">{student.profile.emergency_contact || '-'}</dd>
                </div>
                <div className="sm:col-span-2">
                  <dt className="text-sm font-medium text-gray-500">주소</dt>
                  <dd className="mt-1 text-sm text-gray-900">{student.profile.address || '-'}</dd>
                </div>
                <div className="sm:col-span-2">
                  <dt className="text-sm font-medium text-gray-500">특이사항</dt>
                  <dd className="mt-1 text-sm text-gray-900">{student.profile.note || '-'}</dd>
                </div>
              </dl>
            </div>
          </div>
        ...
        {currentTab === '#subscriptions' && (
          <div className="bg-white shadow sm:rounded-lg">
              ...
              {subscriptions.map((subscription) => (
                <li key={subscription.id} className="px-4 py-4 sm:px-6">
                  <div className="flex items-center justify-between">
                    <div>
                      <p className="text-sm font-medium text-primary-600">{subscription.dance_class}</p>
                      <p className="text-sm text-gray-500">
                        {subscription.subscription_type === 'days' ? '기간제' : '횟수제'}
                      </p>
                    </div>
            ...
            {subscriptions.length === 0 && (
              <p className="py-4 text-center text-sm text-gray-500">수강권 정보가 없습니다.</p>
            )}
          </div>
        )}
              ...
              <tbody className="bg-white divide-y divide-gray-200">
                {attendance.map((record) => (
                  <tr key={record.id}>
                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{record.dance_class}</td>
                    <td className="px-6 py-4 whitespace-nowrap">
                      ...
                      >
                        {record.status === 'present' ? '출석' : 
                         record.status === 'absent' ? '결석' : '지각'}
                      </span>
                    </td>
                    <td className="px-6 py-4 text-sm text-gray-500">{record.memo || '-'}</td>
                  </tr>
                ))}
              </tbody>
            </table>
            {attendance.length === 0 && (
              <p className="py-4 text-center text-sm text-gray-500">출석 기록이 없습니다.</p>
            )}
          </div>
        )}
      </div>
